    ...
    internal class Program
    {
        ...
        static async Task RunInteractiveLoopAsync(ManagerAgent agent, ConsoleUI ui)
        {
            while (true)
            {
                ...
                string input = OrchX.UI.ConsoleInputHelper.ReadConsoleInput(CommandManager.GetRegisteredCommandNames());
                if (input == null) break;
        ...
        static async Task UpdateEnvWithModelListAsync(string apiKey)
        {
            ...
            try
            {
                ...
                var tempClient = new OrchX.AIClient.GeminiClient(apiKey);
